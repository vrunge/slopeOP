...
algoChpt <- function(fileCSV, algo = "slopeOP", states = 0:80, beta = 2, columns = NULL)
{
  ...
  for(i in columns)
  {
    ...
    elapsed_times[i] <- as.numeric(difftime(end_time, start_time, units=c('secs'))) # store elapsed time in seconds
  }


  out_fname <- paste0("simu_",substr(fileCSV,1,nchar(fileCSV)-4),"_",algo,"_",sprintf("%.3f", beta))
  
  write.csv(x = dfresponse, file = paste0(out_fname,'.csv'))
  write.csv(x = elapsed_times, file = paste0(out_fname, '_time', '.csv'))
